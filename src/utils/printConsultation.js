// src/utils/printConsultation.js

// Utility function to format date with Urdu month name
const urduDate = (date) => {
    const urduMonths = [
      "جنوری", // January
      "فروری", // February
    ...
    const day = d.getDate(); // No padding for natural look (e.g., 25, not 05)
    const month = urduMonths[d.getMonth()]; // Get Urdu month name
    const year = d.getFullYear();
    return `${day} ${month} ${year}`; // e.g., "25 مارچ 2025"
  };
                ...
                <tbody>
                  ${selectedMedicines
                    .map((med) => {
                      ...
                      return `
                        <tr>
                          <td>${medicineData?.label || "Unknown Medicine"}</td>
