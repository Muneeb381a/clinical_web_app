...
// FullPageLoader component remains unchanged
    ...
    <div className="text-center space-y-6">
      ...
      >
        ...
        <motion.div
          ...
          animate={{ rotate: [0, 180], opacity: [0.8, 1] }}
          transition={{
...
const PatientSearch = () => {
  ...
  const onSearch = async (data) => {
    ...
    setSearchedMobile(isMobile ? searchInput : "");
    setSearchedName(isName ? searchInput : "");
    ...
    try {
      ...
      const patientRes = await fetchWithRetry(
        ...
        (data) => {
          if (!data?.success) {
            console.warn("Invalid search response:", data);
  ...
  const handleEditClick = (consultationId) =>
    navigate(`/patients/${patient.id || patient._id}/consultations/${consultationId}/edit`);
  ...
  return (
    <CustomErrorBoundary navigate={navigate}>
      <div className="min-h-screen p-8 relative overflow-hidden isolate w-[90vw] mx-auto before:absolute before:inset-0 before:bg-gradient-to-br before:from-white before:-z-10">
        ...
        {isAddingConsultation && <FullPageLoader message="Loading new consultation..." />}
        ...
        >
          ...
          {!patient && patientsList.length === 0 && !showAddPatient && (
            <PatientSearchForm onSearch={onSearch} isSearching={isSearching} />
          )}
            ...
            <div className="space-y-8">
              <div className="bg-gradient-to-br from-blue-50/80 to-purple-50/80 p-8 rounded-2xl border border-white/20 shadow-xl backdrop-blur-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  {[
                    { label: "Patient ID", value: patient.id || patient._id, icon: FaIdCard },
                    { label: "Full Name", value: patient.name || "Unknown", icon: FaUser },
                    { label: "Mobile", value: patient.mobile || "N/A", icon: FaPhone },
                  ].map((item, index) => (
                    <motion.div
                      key={item.label}
                      initial={{ opacity: 0, y: 10 }}
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center">
                  <motion.button
                    ...
                    className={`relative bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-xl shadow-lg transition-all flex items-center gap-3 w-full md:w-auto justify-center overflow-hidden cursor-pointer ${isAddingConsultation ? "opacity-80 cursor-not-allowed" : ""}`}
                  >
              ...
              <div>
                ...
                {consultations.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {consultations
                      .filter((consultation) => consultation.visit_date && new Date(consultation.visit_date).getFullYear() > 1970)
          ...
          {showAddPatient && !patient && patientsList.length === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
